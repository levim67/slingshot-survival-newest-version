...
export const renderGame = (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement, state: GameState, upgrades: Upgrades, baseZoom: number, noisePattern: CanvasPattern | null) => {
    ...
    state.world.entities.forEach(e => {
        ...
        if (e.type === 'bomb') {
            // Shadows okay for single bomb
            ctx.shadowColor = '#000'; ctx.shadowBlur = 5; ctx.fillStyle = '#111'; ctx.beginPath(); ctx.arc(0, 0, e.radius, 0, Math.PI*2); ctx.fill();
            ctx.fillStyle = '#333'; ctx.beginPath(); ctx.arc(-5, -5, e.radius * 0.4, 0, Math.PI*2); ctx.fill();
            ctx.fillStyle = '#444'; ctx.fillRect(-4, -e.radius - 4, 8, 6);
            ctx.strokeStyle = '#a87932'; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(0, -e.radius - 4); ctx.quadraticCurveTo(5, -e.radius - 10, 10, -e.radius - 8); ctx.stroke();
        } 
...
const renderBoss = (ctx: CanvasRenderingContext2D, boss: Entity, time: number) => {
    ...
    if (boss.bossData?.type === 'WORM_DEVOURER') {
        ...
        if(boss.bossData.invincibilityTimer>0) {ctx.globalAlpha=0.5;ctx.fillStyle='#fff';} else ctx.fillStyle=boss.color;
        ctx.beginPath(); ctx.ellipse(0,0,boss.radius, boss.radius*0.8, 0,0,Math.PI*2); ctx.fill();
        ctx.fillStyle='rgba(0,0,0,0.3)';ctx.beginPath();ctx.arc(0,-boss.radius*0.3,boss.radius*0.4,0,Math.PI*2);ctx.fill();
        if(isHead){
            ctx.fillStyle='#eab308';const b=Math.sin(time*10)*10;
            ctx.beginPath();ctx.moveTo(boss.radius*0.5,boss.radius*0.5);ctx.lineTo(boss.radius+20,20+b);ctx.lineTo(boss.radius,40);ctx.fill();
            ctx.beginPath();ctx.moveTo(boss.radius*0.5,-boss.radius*0.5);ctx.lineTo(boss.radius+20,-20-b);ctx.lineTo(boss.radius,-40);ctx.fill();
            ctx.fillStyle='#ef4444';ctx.beginPath();ctx.arc(10,-20,8,0,Math.PI*2);ctx.arc(10,20,8,0,Math.PI*2);ctx.fill();
        }
    } else {
        // Cube
        const r = boss.radius;
        const state = boss.bossData!.state;
        const col = state==='IDLE_VULNERABLE'?'#4ade80':state==='ALIGNING'?'#facc15':'#ef4444';
        const scale = 1+Math.sin(time*5)*0.05; ctx.scale(scale,scale);
        if(boss.bossData!.invincibilityTimer>0){ctx.globalAlpha=0.5;ctx.fillStyle='#fff';ctx.beginPath();ctx.arc(0,0,r*1.6,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1;}
        if(state==='SHOOTING'){ctx.beginPath();ctx.moveTo(20,0);ctx.lineTo(800,0);ctx.strokeStyle='rgba(255,0,0,0.5)';ctx.setLineDash([20,20]);ctx.stroke();ctx.setLineDash([]);}
        // Reduced shadow
        ctx.strokeStyle=col;ctx.lineWidth=8;ctx.shadowColor=col;ctx.shadowBlur=10;
        ctx.strokeRect(-r,-r,r*2,r*2);
        ctx.fillStyle='#000';ctx.fillRect(-r+10,-r+10,r*2-20,r*2-20);
        ctx.fillStyle=col;ctx.beginPath();ctx.arc(0,0,r*0.4,0,Math.PI*2);ctx.fill();
    }
    ctx.restore();
};
