...
const Shop: React.FC<ShopProps> = ({
    ...
    const isStormfireUnlocked = (upgradeLevels['auto_fireball'] || 0) >= 9 && (upgradeLevels['chain_lightning'] || 0) >= 9;
    const anyParagonAccessible = isParagonUnlocked || isStormfireUnlocked;
        ...
        <div className="absolute inset-0 flex flex-col items-center bg-black/95 z-20 backdrop-blur-md font-sans text-white pt-8 pb-4">
            ...
            <div className="w-full max-w-lg px-6 flex gap-2 mb-6 overflow-x-auto no-scrollbar">
                {['ESSENTIALS', 'UTILITY', 'PERMANENT', 'PARAGON', 'COSMETICS'].map((tab) => (
                    ...
                    ${tab === 'PARAGON' && !anyParagonAccessible ? 'opacity-50 grayscale' : ''}
                `}
                    >
                        {tab.replace('_', ' ')} {tab === 'PARAGON' && !anyParagonAccessible && <Lock size={10} className="inline ml-1" />}
                    </button>
                ))}
            </div>
            ...
            <div className="flex-1 w-full max-w-lg overflow-y-auto px-6 space-y-4 pb-20 mask-gradient-bottom">
                ...
                {/* PARAGON LOCK SCREEN */}
                {activeTab === 'PARAGON' && !anyParagonAccessible ? (
                    <div className="flex flex-col items-center justify-center text-center p-6 border border-white/10 rounded-xl bg-slate-900/50">
                        ...
                        <div className="flex flex-col gap-2 w-full text-xs font-mono">
                            <div className="text-white/40 uppercase tracking-wider text-[10px] mt-2 mb-1">Super Missile Requirements</div>
                            <div className={`flex justify-between p-2 rounded ${(upgradeLevels['missiles'] || 0) >= 9 ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}`}>
                                <span>Auto Missiles Lv 9</span>
                                <span>{upgradeLevels['missiles'] || 0}/9</span>
                            </div>
                            <div className={`flex justify-between p-2 rounded ${(upgradeLevels['bounce_missile'] || 0) >= 9 ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}`}>
                                <span>Bounce Missile Lv 9</span>
                                <span>{upgradeLevels['bounce_missile'] || 0}/9</span>
                            </div>
                            <div className="text-white/40 uppercase tracking-wider text-[10px] mt-4 mb-1">Stormfire Requirements</div>
                            <div className={`flex justify-between p-2 rounded ${(upgradeLevels['auto_fireball'] || 0) >= 9 ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}`}>
                                <span>Auto Fireball Lv 9</span>
                                <span>{upgradeLevels['auto_fireball'] || 0}/9</span>
                            </div>
                            <div className={`flex justify-between p-2 rounded ${(upgradeLevels['chain_lightning'] || 0) >= 9 ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}`}>
                                <span>Chain Lightning Lv 9</span>
                                <span>{upgradeLevels['chain_lightning'] || 0}/9</span>
                            </div>
                        </div>
                    </div>
                ) : (
                    ...
                    (activeTab === 'ESSENTIALS' || activeTab === 'UTILITY' || activeTab === 'PERMANENT' || activeTab === 'PARAGON') && (
                        <div className="space-y-4">
                            {UPGRADE_ITEMS.filter(i => i.category === activeTab).map((item) => {
                                ...
                                // Check Paragon-specific requirements
                                let itemUnlocked = true;
                                let requirementText = '';
                                if (item.id === 'paragon_super_missile') {
                                    itemUnlocked = isParagonUnlocked;
                                    if (!itemUnlocked) requirementText = 'Requires Auto Missiles Lv 9 and Bounce Missile Lv 9';
                                } else if (item.id === 'paragon_stormfire') {
                                    itemUnlocked = isStormfireUnlocked;
                                    if (!itemUnlocked) requirementText = 'Requires Auto Fireball Lv 9 and Chain Lightning Lv 9';
                                }

                                const canPurchase = canAfford && itemUnlocked;
                                ...
                                return (
                                    ...
                                    ${isParagon && !itemUnlocked ? 'opacity-60' : ''}
                                `}
                                    >
                                        ...
                                        `}>
                                                ...
                                                <div>
                                                    ...
                                                    {/* Show requirement text for locked Paragon items */}
                                                    {isParagon && !itemUnlocked && requirementText && (
                                                        <p className="text-[10px] text-red-400/80 mt-2 flex items-center gap-1">
                                                            <Lock size={10} /> {requirementText}
                                                        </p>
                                                    )}
                                                </div>
                                            </div>
                                            ...
                                            {item.maxLevel > 1 ? (
                                                ...
                                                <div className="flex items-center gap-2 mb-1">
                                                    {isMaxed ?
                                                        ...
                                                        :
                                                        <span className="text-white/30 text-xs font-bold uppercase tracking-widest flex items-center gap-1"><Lock size={12} /> {!itemUnlocked ? 'Requirements Not Met' : 'Locked'}</span>
                                                    }
                                                </div>
                                            )}
